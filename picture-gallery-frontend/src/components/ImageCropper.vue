<template>
  ...
  >
    ...
    <div class="image-edit-actions" v-if="isTeamSpace">
      <a-space>
        <a-tag v-if="editingUser" color="orange" class="tech-tag">
          <template #icon><loading-outlined /></template>
          {{ editingUser.userName }} 正在编辑
        </a-tag>
        <a-button v-if="canEnterEdit" type="primary" ghost @click="enterEdit" class="tech-btn">进入编辑</a-button>
        <a-button v-if="canExitEdit" danger ghost @click="exitEdit" class="tech-btn">退出编辑</a-button>
      </a-space>
    </div>
    <div style="margin-bottom: 16px" />
    <!-- 图片操作 -->
...
// 缩放比例
const changeScale = (num, isRemote = false) => {
...
// 向左旋转
const rotateLeft = (isRemote = false) => {
...
// 向右旋转
const rotateRight = (isRemote = false) => {
